import { w as Se, o as je, f as _t } from './DcxtdZEe.js';
import { a8 as T, h as x, T as P } from './BdEMnGkn.js';
class ce {
	constructor(t, n) {
		((this.status = t),
			typeof n == 'string'
...
const U = globalThis.__sveltekit_2bqhci?.base ?? '/Locked-In',
	Pt = globalThis.__sveltekit_2bqhci?.assets ?? U,
	Ct = '1753723398232',
...
async function Sn(e, t, n) {
	...
	(n
		...
		: (await W({
				type: 'enter',
				url: ze(v.hash ? mn(new URL(location.href)) : location.href),
				replace_state: !0
			}),
			a()),
		pn());
...
async function ft({ id: e, invalidating: t, url: n, params: r, route: a, preload: s }) {
	...
	const m = f.map((g, y) => {
		const b = w.branch[y],
			A = !!g?.[0] && (b?.loader !== g[1] || Ge(p, u, h, l, b.server?.uses, r));
		return (A && (p = !0), A);
	});
	if (m.some(Boolean)) {
		try {
	...
	const E = f.map(async (g, y) => {
		...
		const b = w.branch[y],
			A = _?.[y];
		if ((!A || A.type === 'skip') && g[1] === b?.loader && !Ge(R, u, h, l, b.universal?.uses, r))
			return b;
...
async function W({
	...
	(f(),
		(X = !0),
		oe && l.navigation.type !== 'enter' && N.navigating.set((J.current = l.navigation)));
	let _ = u && (await ft(u));
	if (!_) {
...
export { Sn as a, vn as l, S as p, N as s };
