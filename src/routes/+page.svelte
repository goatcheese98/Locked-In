<script lang="ts">
  ...
  function parseHslString(hslStr: string | null | undefined): { h: number, s: number, l: number } | null {
  ...
  let targetH1 = 0, targetS1 = 0, targetL1 = 0;
  let startH1 = 0, startS1 = 0, startL1 = 0;
  let cycleStartTime1: number | undefined;

  let targetH2 = 0, targetS2 = 0, targetL2 = 0;
  let startH2 = 0, startS2 = 0, startL2 = 0;
  ...
  function handleClickOutside(event: MouseEvent) {
    if (showSettings) {
      if (settingsPanelElement && !settingsPanelElement.contains(event.target as Node) &&
          settingsButtonElement && !settingsButtonElement.contains(event.target as Node)) {
        showSettings = false;
      }
    }
  }
  ...
  $: {
    ...
    const progressRatio = state.isRunning || state.timeRemaining < totalDurationForCurrentSession 
  ...
  $: if (hasMounted) {
    ...
    if ($timerSettings.workDuration !== newTimerSettings.workDuration ||
  ...
  timerSettings.subscribe(settings => {
    if (settings.workDuration / 60 !== localWorkDuration) localWorkDuration = settings.workDuration / 60;
    if (settings.shortBreakDuration / 60 !== localShortBreakDuration) localShortBreakDuration = settings.shortBreakDuration / 60;
    if (settings.longBreakDuration / 60 !== localLongBreakDuration) localLongBreakDuration = settings.longBreakDuration / 60;
    if (settings.sessionsBeforeLongBreak !== localSessionsBeforeLongBreak) localSessionsBeforeLongBreak = settings.sessionsBeforeLongBreak;
  ...
  function handlePrev1Click() {
    ...
    if (sliderInteractionTimeoutId1) { // Clear pending slider-resume
      clearTimeout(sliderInteractionTimeoutId1);
      sliderInteractionTimeoutId1 = undefined;
    }
...
<InteractiveBackground gradientColor1={$backgroundSettings.gradientColor1} gradientColor2={$backgroundSettings.gradientColor2} />

<div class="min-h-screen bg-transparent text-gray-100 flex flex-col items-center justify-center p-4 selection:bg-teal-500 selection:text-teal-900 relative z-0" style="user-select: none; -webkit-user-select: none; -ms-user-select: none;">
  ...
  <div class="fixed top-4 right-4 z-20 flex space-x-2">
    ...
    <button
      ...
      class="p-2 bg-white/5 hover:bg-white/10 border border-slate-400/20 rounded-full text-gray-300 hover:text-teal-300 transition-all shadow-lg backdrop-blur-sm"
  ...
  >
    <div 
      class="settings-panel-content p-6 rounded-lg shadow-xl backdrop-blur-lg" 
    ...
    >
      ...
      <h3 class="text-xl font-semibold text-teal-400 mb-6 border-b border-slate-600 pb-3">Background Effects</h3>
      <BackgroundSettingsPanel 
        ...
        canGoNext1={$backgroundSettings.gradientColor1HistoryIndex < $backgroundSettings.gradientColor1History.length - 1}
        ...
        canGoNext2={$backgroundSettings.gradientColor2HistoryIndex < $backgroundSettings.gradientColor2History.length - 1}
      ...
      <h3 class="text-xl font-semibold text-teal-400 mt-8 mb-6 border-b border-slate-600 pb-3">Timer Settings</h3>
      <form on:submit|preventDefault class="space-y-6 mb-8">
        ...
        <div>
          ...
          <input type="number" id="work-duration" bind:value={localWorkDuration} min="1" class="mt-1 block w-full bg-slate-600/80 border-slate-500/70 rounded-md shadow-sm py-2 px-3 text-gray-200 focus:outline-none focus:ring-teal-500 focus:border-teal-500 sm:text-sm">
        </div>
        <div>
          <label for="short-break-duration" class="block text-sm font-medium text-gray-300 mb-1">Short Break (minutes)</label>
          <input type="number" id="short-break-duration" bind:value={localShortBreakDuration} min="1" class="mt-1 block w-full bg-slate-600/80 border-slate-500/70 rounded-md shadow-sm py-2 px-3 text-gray-200 focus:outline-none focus:ring-teal-500 focus:border-teal-500 sm:text-sm">
        </div>
        <div>
          <label for="long-break-duration" class="block text-sm font-medium text-gray-300 mb-1">Long Break (minutes)</label>
          <input type="number" id="long-break-duration" bind:value={localLongBreakDuration} min="1" class="mt-1 block w-full bg-slate-600/80 border-slate-500/70 rounded-md shadow-sm py-2 px-3 text-gray-200 focus:outline-none focus:ring-teal-500 focus:border-teal-500 sm:text-sm">
        </div>
        <div>
          <label for="sessions-before-long-break" class="block text-sm font-medium text-gray-300 mb-1">Sessions before Long Break</label>
          <input type="number" id="sessions-before-long-break" bind:value={localSessionsBeforeLongBreak} min="1" class="mt-1 block w-full bg-slate-600/80 border-slate-500/70 rounded-md shadow-sm py-2 px-3 text-gray-200 focus:outline-none focus:ring-teal-500 focus:border-teal-500 sm:text-sm">
        </div>
        <div class="flex items-center mt-6">
          <input id="auto-cycle" type="checkbox" bind:checked={localAutoCycle} class="h-4 w-4 text-teal-600 border-slate-500/70 rounded focus:ring-teal-500 bg-slate-600/80 cursor-pointer">
  ...
  <header class="w-full max-w-md mb-8 text-center">
    <h1 class="text-5xl font-thin tracking-wider {headerTextColorClass} transition-colors duration-500">Locked In</h1>
    <p class="text-sm {subHeaderTextColorClass} mt-1 transition-colors duration-500">Bad Day to Be {currentTaglineSuffix}</p>
  </header>

  <main 
    class="w-full max-w-md rounded-lg px-6 pt-4 pb-6 border border-slate-400/20 transition-colors duration-1000 ease-linear transition-shadow duration-[2000ms] ease-out relative overflow-hidden shadow-[0_15px_25px_-5px_rgba(0,0,0,0.3),_0_8px_10px_-6px_rgba(0,0,0,0.2),_inset_0_1px_0px_hsla(0,0%,100%,0.07),_inset_0_0px_1px_hsla(0,0%,100%,0.05)] hover:shadow-[0_20px_30px_-5px_rgba(0,0,0,0.35),_0_10px_15px_-6px_rgba(0,0,0,0.25),_inset_0_1px_0px_hsla(0,0%,100%,0.07),_inset_0_0px_1px_hsla(0,0%,100%,0.05)]"
