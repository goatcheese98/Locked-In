...
const initialState: TimerState = { // Define initialState after reevaluatedInitialSettings
  currentSession: 'Work',
...
export function startTimer() {
  ...
  if (currentTimeRemaining <= 0) {
    ...
    switch (currentState.currentSession) {
      case 'Work': currentTimeRemaining = settings.workDuration; break;
      case 'Short Break': currentTimeRemaining = settings.shortBreakDuration; break;
      case 'Long Break': currentTimeRemaining = settings.longBreakDuration; break;
    }
  }
...
export function skipSession() {
  ...
  timerState.update(state => {
    ...
    } else { // Currently on a Short Break or Long Break
      nextSession = 'Work';
...
function handleSessionCompletion() {
  ...
  } else { // If current session was Short Break or Long Break
    nextSession = 'Work';
  }
  ...
  if (settings.autoCycle) { // PRD F1.5
    startTimer();
  }
} 