    ...
    if (storedSettings) {
      try {
        ...
        settingsToReturn = { 
          ...settingsToReturn,
          ...parsed,
          isAutoCyclingColor1: parsed.isAutoCyclingColor1 ?? defaultBackgroundSettings.isAutoCyclingColor1,
          isAutoCyclingColor2: parsed.isAutoCyclingColor2 ?? defaultBackgroundSettings.isAutoCyclingColor2,
        };
        ...
        if (!parsed.gradientColor1History || parsed.gradientColor1History.length === 0) {
          ...
          settingsToReturn.gradientColor1HistoryIndex = Math.max(0, Math.min(parsed.gradientColor1HistoryIndex ?? 0, parsed.gradientColor1History.length - 1));
          settingsToReturn.gradientColor1 = parsed.gradientColor1History[settingsToReturn.gradientColor1HistoryIndex];
        }
        ...
        if (!parsed.gradientColor2History || parsed.gradientColor2History.length === 0) {
          ...
          settingsToReturn.gradientColor2HistoryIndex = Math.max(0, Math.min(parsed.gradientColor2HistoryIndex ?? 0, parsed.gradientColor2History.length - 1));
          settingsToReturn.gradientColor2 = parsed.gradientColor2History[settingsToReturn.gradientColor2HistoryIndex];
        }
...
function pushColorToHistory(newColor: string, currentHistory: string[], currentIndex: number): { history: string[], index: number } {
...
export function setGradientColor1(newColor: string) {
  ...
  const { history, index } = pushColorToHistory(newColor, current.gradientColor1History, current.gradientColor1HistoryIndex);
...
export function setGradientColor2(newColor: string) {
  ...
  const { history, index } = pushColorToHistory(newColor, current.gradientColor2History, current.gradientColor2HistoryIndex);
  backgroundSettings.update(settings => ({
    ...settings,
