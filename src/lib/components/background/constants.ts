...
export const RIPPLE_CONFIG = {
  ...
  click: { // Config for ripples created by a mouse click.
    maxRadius: 85,       
    speed: 0.9,           
    lineWidth: 2.2,       
    initialOpacity: 0.95,  
  ...
  ambient: { // Config for background, automatically generated ripples.
    maxRadius: 30, 
    speed: 0.2, 
    lineWidth: 1, 
    initialOpacity: 0.15, 
  ...
  longPress: { // Config for ripples created by a long mouse press.
    maxRadius: 150,       
    speed: 0.3,           
    lineWidth: 3.5,       
    initialOpacity: 0.85, 
  ...
  glintExpire: { // Config for ripples created when a regular Glint expires.
    maxRadius: 40, 
    speed: 0.9, 
    lineWidth: 1.0,
    initialOpacity: 0.6,
...
export const WAVE_SHIMMER_MAX_OPACITY = 0.30; 
...
export const RARE_SIGHTING_CONFIGS: Record<RareSighting['type'], RareSightingConfig> = {
  plane: { 
    minSpeed: 0.25, maxSpeed: 0.5,   
    minSize: 25, maxSize: 45,   
    baseOpacity: 0.4, 
    minDuration: 10000, maxDuration: 20000, // Note: Durations seem very long (in frames), maybe intended in ms?
    chancePerInterval: 0.01 
  },
  whaleShadow: { 
    minSpeed: 0.18, maxSpeed: 0.32,  
    minSize: 40, maxSize: 70,  
    baseOpacity: 0.50,  
    minDuration: 1200, maxDuration: 2500, // Duration in frames
