...
import {
    MAX_FOAM_STREAKS, FOAM_STREAK_COLOR_PRIMARY, FOAM_STREAK_COLOR_SECONDARY,
    FOAM_PARTICLE_MIN_SIZE, FOAM_PARTICLE_MAX_SIZE, FOAM_STREAK_MAX_WIDTH,
    FOAM_STREAK_SEGMENT_LENGTH_MIN, FOAM_STREAK_SEGMENT_LENGTH_MAX, FOAM_STREAK_LIFESPAN_MIN,
    FOAM_STREAK_LIFESPAN_MAX, FOAM_STREAK_FADE_DURATION_RATIO, FOAM_STREAK_DRIFT_X_MIN,
    FOAM_STREAK_DRIFT_X_MAX, FOAM_STREAK_DRIFT_Y_MIN, FOAM_STREAK_DRIFT_Y_MAX,
    FOAM_STREAK_WOBBLE_FACTOR, FOAM_PARTICLE_APPEAR_DELAY_PER_PARTICLE, FOAM_STREAK_JAGGEDNESS,
    FOAM_STREAK_MAIN_PATH_SEGMENTS_MIN, FOAM_STREAK_MAIN_PATH_SEGMENTS_MAX, FOAM_BRANCH_CHANCE,
    FOAM_SUB_BRANCH_MAX_SEGMENTS, FOAM_SUB_BRANCH_ANGLE_OFFSET, FOAM_PARTICLES_PER_SEGMENT_POINT,
    FOAM_PARTICLE_LIFESPAN_MIN, FOAM_PARTICLE_LIFESPAN_MAX, FOAM_PARTICLE_FADE_RATIO
} from '../constants';
import {
    addFoamStreak, getFoamStreaks, releaseFoamStreak,
    addFoamParticle, releaseFoamParticle
} from '../animationState';
...
export function createFoamStreak(canvasWidth: number, canvasHeight: number): void {
    ...
    if (startSide === 0) { // Top
        ...
        initialVy = FOAM_STREAK_DRIFT_Y_MIN + Math.random() * (FOAM_STREAK_DRIFT_Y_MAX - FOAM_STREAK_DRIFT_Y_MIN);
        ...
        initialVx = FOAM_STREAK_DRIFT_X_MIN + Math.random() * (FOAM_STREAK_DRIFT_X_MAX - FOAM_STREAK_DRIFT_X_MIN);
     ...
     */
    const generatePathParticles = (targetStreak: FoamStreak, startX: number, startY: number, pathAngle: number, numSegments: number, isSubBranch: boolean) => {
        let currentX = startX;
        let currentY = startY;
        let currentAngle = pathAngle;

        for (let i = 0; i < numSegments; i++) {
            const segmentLength = FOAM_STREAK_SEGMENT_LENGTH_MIN + Math.random() * (FOAM_STREAK_SEGMENT_LENGTH_MAX - FOAM_STREAK_SEGMENT_LENGTH_MIN);
            ...
            } else { // Main path angle constraints based on start side
                if (startSide === 0) currentAngle = Math.max(Math.PI / 4, Math.min(3 * Math.PI / 4, currentAngle));
                else if (startSide === 1) currentAngle = Math.max(-Math.PI / 3, Math.min(Math.PI / 3, currentAngle));
            ...
            for (let k = 0; k < FOAM_PARTICLES_PER_SEGMENT_POINT; k++) {
                const particleSize = FOAM_PARTICLE_MIN_SIZE + Math.random() * (FOAM_PARTICLE_MAX_SIZE - FOAM_PARTICLE_MIN_SIZE);
                ...
                addFoamParticle(targetStreak, {
                    ...
                    particleAge: 0,
                    particleMaxAge: FOAM_PARTICLE_LIFESPAN_MIN + Math.random() * (FOAM_PARTICLE_LIFESPAN_MAX - FOAM_PARTICLE_LIFESPAN_MIN)
                });
            }
            ...
            if (!isSubBranch && Math.random() < FOAM_BRANCH_CHANCE && i < numSegments - FOAM_SUB_BRANCH_MAX_SEGMENTS / 2 && i > 2) { // Avoid branching too early or late
                const branchAngleOffset = (Math.random() < 0.5 ? -1 : 1) * FOAM_SUB_BRANCH_ANGLE_OFFSET * (0.7 + Math.random() * 0.6);
                const subBranchNumSegments = Math.floor(FOAM_SUB_BRANCH_MAX_SEGMENTS * (0.5 + Math.random() * 0.5));
                generatePathParticles(targetStreak, currentX, currentY, currentAngle + branchAngleOffset, subBranchNumSegments, true);
            }
    ...
    const mainPathSegments = FOAM_STREAK_MAIN_PATH_SEGMENTS_MIN + Math.floor(Math.random() * (FOAM_STREAK_MAIN_PATH_SEGMENTS_MAX - FOAM_STREAK_MAIN_PATH_SEGMENTS_MIN + 1));
    ...
    const newStreakProps = {
        ...
        opacity: 0, // Start invisible
        age: 0,
        maxAge: FOAM_STREAK_LIFESPAN_MIN + Math.random() * (FOAM_STREAK_LIFESPAN_MAX - FOAM_STREAK_LIFESPAN_MIN),
    ...
    const addedStreak = getFoamStreaks().find(s => s.anchorX === initialAnchorX && s.anchorY === initialAnchorY && s.age === 0);
 ...
 */
export function updateAndDrawFoamStreaks(ctx: CanvasRenderingContext2D, canvasWidth: number, canvasHeight: number): void {
    ...
    for (let i = currentFoamStreaks.length - 1; i >= 0; i--) {
        ...
        // Basic anchor check
        if (streak.anchorX > -checkBoundsX && streak.anchorX < canvasWidth + checkBoundsX &&
            streak.anchorY > -checkBoundsY && streak.anchorY < canvasHeight + checkBoundsY) {
            ...
            isCompletelyOffScreen = true; 
             for (const p of streak.particles) {
                 ...
                 if (absoluteX > -p.size && absoluteX < canvasWidth + p.size &&
                     absoluteY > -p.size && absoluteY < canvasHeight + p.size) { 
                    isCompletelyOffScreen = false;
                    break; 
                }
            }
        ...
        for (let pIdx = streak.particles.length - 1; pIdx >= 0; pIdx--) {
            ...
            if (particle.particleAge > particle.particleMaxAge) {
                ...
                if (particle.particleAge < particleFadeTime) { 
                    ...
                    particle.opacity = (1 - (particle.particleAge - (particle.particleMaxAge - particleFadeTime)) / particleFadeTime) * particle.maxOpacity;
                } else { 
