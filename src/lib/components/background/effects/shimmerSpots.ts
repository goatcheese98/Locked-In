 ...
 */
import {
    MAX_SHIMMER_SPOTS, SHIMMER_SPAWN_AREA_WIDTH_RATIO, SHIMMER_SPAWN_AREA_HEIGHT_RATIO,
    WAVE_SHIMMER_MIN_SPEED_FACTOR, WAVE_SHIMMER_MAX_SPEED_FACTOR, WAVE_SHIMMER_MIN_OPACITY,
    WAVE_SHIMMER_MAX_OPACITY, WAVE_SHIMMER_COLOR_LIGHTNESS_MIN, WAVE_SHIMMER_COLOR_LIGHTNESS_MAX,
    SHIMMER_RESIZE_MIN_SPEED_MULTIPLIER, SHIMMER_RESIZE_MAX_SPEED_MULTIPLIER
} from '../constants';
import {
    addShimmerSpot, getShimmerSpots,
 ...
 */
export function createShimmerSpot(canvasWidth: number, canvasHeight: number, isWaveType: boolean): void {
 ...
 */
export function updateAndDrawShimmerSpots(ctx: CanvasRenderingContext2D, canvasWidth: number, canvasHeight: number, globalTime: number): void {
...
export function handleResizeShimmerSpots(canvasWidth: number, canvasHeight: number): void {
    ...
    const maxDistInSpawn = Math.sqrt(Math.pow(newSpawnWidth / 2, 2) + Math.pow(newSpawnHeight / 2, 2)) + 0.001; 
    ...
    for (const spot of currentShimmerSpots) {
        ...
        if (spot.baseVx !== undefined && spot.baseVy !== undefined) {
            const distFromSpawnCenter = Math.sqrt(Math.pow(spot.x - newSpawnCenterX, 2) + Math.pow(spot.y - newSpawnCenterY, 2));
            ...
            const currentSpeedMultiplier = SHIMMER_RESIZE_MIN_SPEED_MULTIPLIER + 
                                           normalizedDistance * (SHIMMER_RESIZE_MAX_SPEED_MULTIPLIER - SHIMMER_RESIZE_MIN_SPEED_MULTIPLIER);
