 ...
 */
export function createRareSighting(
	canvasWidth: number,
	canvasHeight: number,
	typeToCreate: keyof typeof RARE_SIGHTING_CONFIGS
): void {
 ...
 */
export function updateAndDrawRareSightings(
	...
	for (let i = currentRareSightings.length - 1; i >= 0; i--) {
		...
		const isOffScreen =
			s.x < -padding ||
			s.x > canvasWidth + padding ||
			s.y < -padding ||
			s.y > canvasHeight + padding;
