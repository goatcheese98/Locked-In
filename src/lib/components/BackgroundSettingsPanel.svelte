<script lang="ts">
  ...
  let { 
    ...
    onPointerDown2,
    onPointerUp2
  } = $props<{ // Define the type inline with $props
    color1H: number;
    color1S: number;
    color1L: number;
  ...
  function handleSliderInput1(event: Event) {
      ...
      let newH = color1H, newS = color1S, newL = color1L;
  ...
  function handleSliderInput2(event: Event) {
      ...
      let newH = color2H, newS = color2S, newL = color2L;
      if (type === 'h') newH = value;
      else if (type === 's') newS = value;
      else if (type === 'l') newL = value;
  ...
  const saturationGradient1 = $derived(`linear-gradient(to right, hsl(${color1H}, 0%, 50%), hsl(${color1H}, 100%, 50%))`); // Use fixed 50% L for preview
  const saturationGradient2 = $derived(`linear-gradient(to right, hsl(${color2H}, 0%, 50%), hsl(${color2H}, 100%, 50%))`); // Use fixed 50% L for preview

</script>

<!-- Moved HTML Form Structure -->
<form on:submit|preventDefault class="space-y-4 mb-8">
  <!-- Gradient Color 1 Controls -->
  <div>
    <label class="block text-sm font-medium text-gray-300 mb-1">Gradient Color 1 (HSL)</label>
    <div class="flex items-center space-x-1.5 mb-1.5">
      ...
      <div 
        class="color-preview"
        style="--preview-bg: {color1HslString};"
      >
      ...
      >
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5"><path fill-rule="evenodd" d="M10.868 2.884c-.321-.772-1.415-.772-1.736 0l-1.83 4.401-4.753.39-3.423 3.252c-.772.732-.297 2.041.678 2.041h4.475l1.83 4.401c.321.772 1.415.772 1.736 0l1.83-4.401h4.475c.975 0 1.45-1.309.678-2.041L12.698 7.675l-4.753-.39-1.83-4.401z" clip-rule="evenodd" /></svg>
      </button>
    </div>
    <div class="grid grid-cols-3 gap-2">
        <div>
          ...
          <input 
            type="range" id="c1-h" bind:value={color1H} on:input={handleSliderInput1} on:pointerdown={onPointerDown1} on:pointerup={onPointerUp1}
            min="0" max="360" class="slider hue-slider" 
        ...
        <div>
          ...
          <input 
            type="range" id="c1-s" bind:value={color1S} on:input={handleSliderInput1} on:pointerdown={onPointerDown1} on:pointerup={onPointerUp1}
            min="0" max="100" class="slider saturation-slider" style="background: {saturationGradient1};"
          >
        </div>
        <div>
          <label for="c1-l" class="block text-xs font-medium text-gray-400">L%</label>
          <input 
            type="range" id="c1-l" bind:value={color1L} on:input={handleSliderInput1} on:pointerdown={onPointerDown1} on:pointerup={onPointerUp1}
            min="0" max="100" class="slider lightness-slider" style="background: linear-gradient(to right, black, white);"
          >
        </div>
    </div>
    <div class="flex items-center pt-2 mt-2 mb-4"> 
  ...
  <div>
    <label class="block text-sm font-medium text-gray-300 mb-1">Gradient Color 2 (HSL)</label>
    <div class="flex items-center space-x-1.5 mb-1.5">
      <button 
        type="button" 
        on:click={onPrev2} 
      ...
      <div 
        class="color-preview"
        style="--preview-bg: {color2HslString};"
      >
        <div class="hsl-values">
    ...
    <div class="grid grid-cols-3 gap-2">
      <div>
        ...
        <input 
          type="range" id="c2-h" bind:value={color2H} on:input={handleSliderInput2} on:pointerdown={onPointerDown2} on:pointerup={onPointerUp2}
          min="0" max="360" class="slider hue-slider" 
          style="background: linear-gradient(to right, hsl(0, 100%, 35%), hsl(60, 100%, 35%), hsl(120, 100%, 35%), hsl(180, 100%, 35%), hsl(240, 100%, 35%), hsl(300, 100%, 35%), hsl(360, 100%, 35%));"
        >
      </div>
      <div>
        ...
        <input 
          type="range" id="c2-s" bind:value={color2S} on:input={handleSliderInput2} on:pointerdown={onPointerDown2} on:pointerup={onPointerUp2}
          min="0" max="100" class="slider saturation-slider" style="background: {saturationGradient2};"
        >
      </div>
      <div>
        <label for="c2-l" class="block text-xs font-medium text-gray-400">L%</label>
        <input 
          type="range" id="c2-l" bind:value={color2L} on:input={handleSliderInput2} on:pointerdown={onPointerDown2} on:pointerup={onPointerUp2}
          min="0" max="100" class="slider lightness-slider" style="background: linear-gradient(to right, black, white);"
        >
      </div>
    </div>
...
<style>
  ...
  .control-button:focus-visible {
    outline: none;
    box-shadow: 0 0 0 2px hsl(215, 27%, 17%), 0 0 0 4px hsl(174, 72%, 56%, 0.6);
  }
  ...
  .custom-checkbox:focus-visible {
    box-shadow: 0 0 0 2px hsl(215, 27%, 17%), 0 0 0 4px hsla(174, 72%, 56%, 0.6);
  }
